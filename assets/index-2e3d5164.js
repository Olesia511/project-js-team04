import"./support-ukraine-6bcca46a.js";const f="https://books-backend.p.goit.global/",z="books/category-list",K="books/top-books",G="books/category",J="books/";async function Y(){return await(await axios.get(`${f}${z}`)).data}async function D(){return await(await axios.get(`${f}${K}`)).data}async function P(t){return await(await axios.get(`${f}${G}?category=${t}`)).data}async function A(t){return await(await axios.get(`${f}${J}${t}`)).data}let w="";const q=document.querySelector(".title-category");function U(t){w=t,P(t).then(e=>{console.log("Get book by category",e),Q(e)}).catch(e=>console.log(e))}function Q(t){const e=document.querySelector(".list-books-by-category"),n=t.map(({book_image:o,title:c,author:i,_id:a})=>`<li data-book-id='${a}' class="element-books-by-category js-target">
      ...
      <p class="title-books-by-category js-target">${i}</p>
        ...
        </h1>`;tt.innerHTML=n+e,document.querySelectorAll(".book-img").forEach(o=>{o.setAttribute("src",o.getAttribute("data-src")),o.onload=()=>o.removeAttribute("data-src")})}).catch(t=>console.log("rej",t))}const y=document.querySelector(".categories-list"),_=document.querySelector(".container-books-by-category"),O=document.querySelector(".top-books-box"),S=document.querySelector('input[type="checkbox"]');let r=null;j();function et(){O.style.display="none"}function T(){O.style.display="flex"}function I(){_.style.display="none"}function ot(){_.style.display="block"}let L,k;Y().then(t=>{const e='<li class="list-categories-item">All categories</li>'+t.map(({list_name:s})=>`<li class="list-categories-item">${s}</li>`).join("");y.insertAdjacentHTML("beforeend",e),r=y.firstChild,nt(),L=document.querySelectorAll(".list-categories-item"),console.log(L),k=[...L],console.log(k),S.checked&&F()}).catch(t=>console.log("rej",t)).finally(()=>{});y.addEventListener("click",st);function st(t){if(t.target.textContent!==r){{if(t.target===t.currentTarget)return;if(t.target===y.firstChild)document.querySelector(".container-best-books")?(v(),T(),I()):(v(),I(),j(),T());else{v(),et();const e=t.target.textContent;console.log(e),U(e),ot()}}r=t.target,r.classList.add("active-list-item")}}function nt(){r.classList.add("active-list-item")}function v(){r.classList.remove("active-list-item")}console.log(S);S.addEventListener("change",ct);function ct(t){t.target.checked?F():at()}function at(){k.map(t=>t.classList.remove("dark"))}function F(){k.map(t=>t.classList.add("dark"))}const it=document.querySelector(".container-books-by-category");console.log();it.addEventListener("click",lt);function lt(t){if(!t.target.classList.contains("see-more-btn"))return;const e=t.target.dataset.category;console.log(e)}const M=document.body,x=document.querySelector(".js-backdrop"),rt=document.querySelector(".close-btn-modal"),W=document.querySelector(".modal"),m=document.querySelector(".btn-add-local"),d=document.querySelector(".add-local"),u=document.querySelector(".remove-local"),g=document.querySelector(".text-box");document.querySelector(".icon-close-modal");const b="user-books";let l;localStorage.getItem(b)?l=JSON.parse(localStorage.getItem(b)):l=[];const R=document.querySelector(".add-books-backend");x.addEventListener("click",dt);rt.addEventListener("click",$);window.addEventListener("keydown",B);function $(){x.classList.add("is-hidden"),W.classList.add("is-hidden"),window.removeEventListener("keydown",B),M.classList.remove("disasble-scroll")}function dt(t){t.currentTarget===t.target&&$()}function B(t){t.code==="Escape"&&$()}const ut=document.querySelector(".list-books-by-category"),gt=document.querySelector(".container-best-books");ut.addEventListener("click",H);gt.addEventListener("click",H);m.addEventListener("click",yt);async function H(t){if(t.target.classList.contains("js-target"))x.classList.remove("is-hidden"),W.classList.remove("is-hidden"),window.addEventListener("keydown",B),M.classList.add("disasble-scroll");else return;const e=t.target.dataset.bookId??t.target.closest("li").dataset.bookId;l.findIndex(s=>s._id===`${e}`)!==-1?(d.hidden=!0,u.hidden=!1,g.classList.remove("text-off")):(d.hidden=!1,u.hidden=!0,g.classList.add("text-off")),await A(e).then(s=>{let o=s.book_image,c=s.title,i=s.description,a=s.author,C=s._id,h=s.buy_links[0].url,p=s.buy_links[1].url;R.innerHTML=mt(o,c,i,a,C,h,p)}).catch(s=>console.log(s))}function mt(t,e,n,s,o,c,i){return`
 ...
 <ul class="modal-img-list">
  ...
  `}async function yt(t){const e=t.target.textContent,n="add to shopping list",s=R.childNodes[1].dataset.book;if(e===n)d.hidden=!1,u.hidden=!0,g.classList.add("text-off"),m.blur(),await A(s).then(o=>l.push({...o})).catch(o=>console.log(o)),console.log(l),await localStorage.setItem(b,JSON.stringify(l)),d.hidden=!0,u.hidden=!1,g.classList.remove("text-off"),m.blur();else{const o=l.findIndex(c=>c._id===s);o!==-1&&(l.splice(o,1),await localStorage.setItem(b,JSON.stringify(l)),d.hidden=!1,u.hidden=!0,g.classList.add("text-off"),m.blur())}}
